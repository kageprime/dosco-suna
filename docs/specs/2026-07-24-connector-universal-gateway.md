# Connector: Universal Connector Gateway

**Status:** Proposed architecture  
**Date:** 2026-07-24  
**Audience:** Dosco platform, API, SDK, CLI, web, security, and infrastructure owners

## 1. Decision

Dosco should replace the current Connector and Connector model with one
...
The user-facing product area is **Connectors**.
...
| Connector | One definition installed in a Dosco project. |
...
| Binding | A session's selection of a connection for a connector. |

The final canonical model does not use `Connector`, `Connector`, or
`Connection`.

## 2. Product Goal

A human or agent must be able to add a previously unknown standards-compliant
API without a Dosco deployment.

The complete flow is:

1. Import or define the API.
2. Discover or configure authentication.
3. Register a Dosco-managed or customer-owned application.
...
Dosco already has most execution-plane primitives:
...
- project, agent, member, subject, and external connections;
- durable session-to-connection bindings;
- `integrations.sh` catalog ingestion;
- SDK, CLI, MCP, and web surfaces.

The current split creates three problems:

1. The configured API and the gateway that calls it use separate product nouns.
2. Connection storage uses a third noun for configured authorization.
3. A new OAuth broker would introduce another adjacent domain.
...
input. Dosco does not acquire or refresh that token.
...
- agent connector grants;
- policies and approvals;
- audit records;
- SSRF protection;
- Pipedream compatibility;
- connection ownership and session-binding invariants.
...
Create an internal `packages/connector` package.
...
service interface so the package can move into a separate Connector deployment
...
- connector definitions and immutable revisions;
- project connectors;
- applications;
- connections;
- encrypted credentials;
...
- connector and connection resolution;
- action lookup;
...
An `ConnectorDefinition` contains:
...
Dosco publishes shared definitions.

Account owners can publish private definitions without a Dosco deployment.
...
Editing a definition creates a new revision. Existing connectors and
connections remain pinned. Migration to another revision is explicit.

### 5.2 Connector

An `Connector` installs one definition revision in a project.
...
- `managed`: Dosco owns the provider-side application;
...
Dosco-managed applications require external provider registration unless the
provider supports dynamic client registration.

### 5.4 Connection

Merge the current connection and credential concepts into `Connection`.
...
Every connector produces one normalized action catalog.
...
needs reviewed Dosco code only when declarative configuration cannot express
its behavior.
...
12. Redirect to an allowlisted Dosco origin.
...
Every HTTP-compatible connector receives a constrained `request` action.
...
3. Resolve the connector alias.
4. Enforce the agent's connector grant.
...
- `/v1/projects/:projectId/connectors/:connectorId/actions`;
- `/v1/projects/:projectId/connectors/:connectorId/connections`;
- `/v1/accounts/:accountId/connector-definitions`;
- `/v1/accounts/:accountId/connector-applications`.

The shared gateway uses:

- `GET /v1/connectors/catalog`;
- `POST /v1/connectors/call`;
- `POST /v1/connectors/request`;
- `GET /v1/connectors/executions/:executionId`.

Keep `/v1/connectors/*` as a temporary translation layer. It calls Connector and
...
Replace agent `connectors` grants with `connectors` grants.
...
manifests emit only `connectors`.
...
- `connector_auth_transactions`;
- `connector_policies`;
- `connector_project_settings`;
- `connector_executions`;
- `session_connector_bindings`.

Canonical column replacements include:

- `connector_id` to `connector_id`;
- `connector_alias` to `connector_alias`;
...
1. Build the canonical Connector domain in an isolated worktree.
...
1. A connector never stores provider credentials.
...
6. Credentials remain inside Connector.
...
1. Create a private connector definition at runtime.
...
13. Complete all steps without a Dosco deployment.
...
- Existing connector IDs and connection IDs remain stable.
...
- Existing Pipedream connectors execute.
- Legacy SDK calls reach connector adapters through deprecated compatibility aliases.
- Canonical code contains no second product noun for connectors or connections.
...
Connector becomes one universal connector gateway.
...
OAuth is one authentication subsystem inside Connector. OpenAPI, Postman,
