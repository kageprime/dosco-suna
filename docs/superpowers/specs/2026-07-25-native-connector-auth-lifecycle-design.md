# Native connector authentication lifecycle
...
- A connection identifies one credential binding.
...
This change does not rename connector, connector, or SDK concepts.
...
An OAuth application is project-scoped or Dosco-managed.
...
1. An authenticated caller starts authorization for one connection.
2. The API checks the caller's connection mutation permission.
3. The API creates a random state value and PKCE verifier.
4. The database stores only the state hash.
5. The database binds the session to the account, project, connection, and user.
...
11. The callback redirects to an allowlisted Dosco web origin.

The state expires after ten minutes. It is valid once.

## Device Authorization lifecycle

1. An authenticated caller starts device authorization for one connection.
...
5. The caller polls the Dosco status route.
...
The connector refreshes a token before its access token expires.
...
- `invalid_grant` marks the connection as `error`.
- Errors never include provider secrets or token values.

## Revocation lifecycle

The connection revoke operation calls the provider revocation endpoint when one is
configured.
...
- The connection status becomes `revoked`.
...
- Callback redirects use configured Dosco web origins only.
...
The connector returns only the current access token to request assembly.
...
Existing connection ownership rules remain authoritative.

- Project connections require connection management permission.
- Member connections can be mutated only by their owner.
- Service accounts cannot impersonate member connection owners.
...
- Connector tests prove access-token injection and refresh locking.
