...
describe('getDeploymentCliInstallCommand', () => {
  ...
  test('on kortix.com the rendered command is unchanged', () => {
    expect(getDeploymentCliInstallCommand('1.2.3', 'https://dosco.example.com')).toBe(
      KORTIX_CLI_INSTALL_COMMAND,
    );
    expect(getDeploymentCliInstallCommand('1.2.3-dev.4', 'https://dosco.example.com')).toBe(
      KORTIX_CLI_DEV_INSTALL_COMMAND,
    );
  });
