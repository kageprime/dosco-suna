...
triggers:
  - slug: nightly-access-review
    ...
    },
    {
      ...
      body: 'Dosco is the Autonomous Company Operating System — open-source, self-hostable, any model. Start one project free.',
    },
  ],
};
...
const introducingKortix: BlogPostEntry = {
  ...
  title: 'Introducing Dosco: the AI command center for your company',
  description:
...
const personalAgentsVsCompanyOs: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'They share Dosco’s core values: open, self-hosted, your models, your data. So why build Dosco? Because a **personal agent** and a **company operating system** are different problems — and stretching one into the other is where it gets painful.',
    },
...
agents:
  ...
  release-bot:
    ...
    {
      type: 'p',
      text: 'That is why Dosco frames the product as an Autonomous Company Operating System, not another assistant with more connectors. A company does not need one more place to paste keys. It needs a Git-backed AI command center where the tools, credentials, policies, and agent work are part of the same owned system.',
    },
    {
      type: 'cta',
  ...
  if (!result.ok) throw new Error(\`close_dispute \${dispute.id}: \${result.reason}\`);
    ...
    },
    {
      ...
      items: [
        ...
        '**You can self-host it.** Dosco is open source. Run it on your own infrastructure with your own keys and your own models. This is not an air-gapped story — `kortix self-host start` pulls images and reaches a sandbox provider over the network — but the data, the config and the model are yours.',
...
const openSourceAiManagementSystem: BlogPostEntry = {
  ...
  title: 'What Dosco actually is: the open-source AI Management System, layer by layer',
  description:
  ...
  blocks: [
    {
      type: 'lead',
      text: 'Dosco is the open-source AI Management System — the leading open-source alternative to Claude Cowork and ChatGPT Work. One git repo holds the agents, the skills, the memory and the connector config. Every session runs on its own isolated machine, on its own branch. Any model, your keys. Work lands through a change request. Self-host it, or run it on our cloud. This is the long version of that paragraph: every layer, in the order you meet it, with the parts we do not claim marked as such.',
    },
...
const theOnlyMoatThatMatters: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'This is the ethos open-source has always represented: platforms that enable more value on top than they capture inside. Dosco is open-source, self-hostable, and model-agnostic by design. We want every company that uses Dosco to build its own compounding advantage — not to make Dosco the only company that gets smarter.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: "Dosco is the open-source AI operating system where your company's knowledge compounds. Connect your tools, deploy an agent, and start accumulating your own token capital. Free to start, free to self-host.",
    },
  ],
};
...
const twoKindsOfCapital: BlogPostEntry = {
  ...
  blocks: [
    ...
    },
    {
      ...
      body: 'Dosco is the open-source AI OS where your company\u2019s knowledge compounds. Free to start, free to self-host, free to own your learning loop.',
    },
  ],
};
...
const theConvergenceOfAiProducts: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Writer.com raised a $200 million series C. They cloned Dosco\u2019s open-source code to build their agent layer. This is not unusual. It is the norm. If you build something useful, someone will copy the architecture. The question is: what happens after they copy it?',
    },
    {
      type: 'callout',
...
const frontierEcosystem: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Dosco is open-source, self-hostable, and model-agnostic for exactly this reason. We want every company to build its own compounding advantage on top of AI, not rent it from someone else. The data flywheel that makes your company better over time should belong to you, not to a model provider in San Francisco.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: 'Dosco is open-source and built for companies that want to own their AI future. Self-host it, connect your own models, and build something that compounds.',
    },
  ],
};
...
const goodBusinessesDontNeedMoats: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'Dosco is open-source. Anyone can clone the repo. Anyone can self-host. Anyone can build a competing product. There is no moat in the VC sense.',
    },
    {
      type: 'p',
