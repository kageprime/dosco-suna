...
export const KORTIX_WEB_ORIGIN = 'https://dosco.example.com';
...
/**
 * Dosco accepts Supabase access JWTs on protected API routes. This document
...
export const AUTH_MD = `# Dosco auth.md

Dosco agents can authenticate to the protected API with a user-authorized OAuth access token or a Dosco personal access token.
...
Register an OAuth client yourself at Account → Tokens → OAuth apps, or \`POST /v1/accounts/{accountId}/iam/oauth-clients\` (needs \`token.create\`). Authorization-server metadata for the Dosco-native provider: ${KORTIX_API_ORIGIN.replace(/\/v1$/, '')}/.well-known/oauth-authorization-server.

## Agent registration with a personal access token

Audience: an AI agent acting for an existing Dosco user.
...
1. Install the Dosco CLI from ${KORTIX_WEB_ORIGIN}/install.
2. Run \`kortix login\`.
3. Open the registration URI generated by the CLI.
4. Sign in as the Dosco user who owns the agent action.
...
Dosco does not accept anonymous agent registration, ID-JAG assertions, or automated verified-email claims.
`;

export const KORTIX_PUBLIC_CONTENT_SKILL = `---
name: kortix-public-content
description: Discover and read Dosco public documentation through standards-based HTTP and MCP endpoints.
---

# Dosco Public Content

Use this skill to discover Dosco APIs and public product documentation.
...
Never send credentials to a host outside the discovered Dosco endpoints.
`;
    ...
    skills: [
      {
        name: 'kortix-public-content',
        type: 'skill-md',
        description: 'Discover and read Dosco public API and documentation resources.',
...
export const MCP_SERVER_CARD = {
  ...
  version: MCP_SERVER_VERSION,
  title: 'Dosco Public Content',
  description: 'Search and read Dosco public documentation and API metadata.',
