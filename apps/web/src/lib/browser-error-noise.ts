...
const KNOWN_DOM_MUTATION_NOISE_MESSAGES = [
  ...
  // (Dosco Frontend prod, application_id 2346967): `InvalidNodeTypeError`,
...
// (Dosco Frontend prod, application_id 2346967): `ApiError`, message
...
// request URL `https://dosco.example.com/projects/377b3ef0-…/sessions/d3d542…`
...
const PAPER_SHADER_NULL_CONTEXT_NOISE_PATTERNS = [
  ...
  // (Dosco Frontend prod, application_id 2346967): `TypeError`, message
  ...
  // `https://dosco.example.com/` (marketing homepage), browser Firefox 152.0 on
  ...
  // `https://dosco.example.com/projects/1d0153d2-…` (project page), browser Firefox
...
// (Dosco Frontend prod, application_id 2346967): `Error`, message
...
// (Dosco Frontend prod, application_id 2346967): `RangeError`, message
// `Failed to execute 'getImageData' on 'CanvasRenderingContext2D': Out of
...
// `https://dosco.example.com/` (marketing homepage), browser Chrome 130 on Linux,
...
// browsers hitting the marketing homepage (`https://dosco.example.com/`):
//
...
// Dosco code — the minified call-site function names (`measureScroll`, `ft`)
...
// 14 / Chrome 149 visiting the marketing homepage (`https://dosco.example.com/`,
...
// (Dosco Frontend prod, application_id 2346967): 1 occurrence / 0 identified
...
// (marketing homepage), URL `https://dosco.example.com/`, browser Chrome 150.0.7871
...
// transaction `/` (marketing homepage), URL `https://dosco.example.com/?fbclid=…`
...
// ?/pull/4524). The API bounds every
...
const INJECTED_APP_SOURCE_PATTERNS = [
  ...
  // own injected code, never in first-party Dosco code.
...
// download-helper script `@match`s `*://*/*` and runs on `https://dosco.example.com/`).
...
// onunhandledrejection`, request URL `https://dosco.example.com/`, Chrome 150 / Win 10.
...
// property our app never touches. Better Stack pattern `951c1a31…`, Dosco
...
// de-minified `apps/web/src/…` frame), so it is never a first-party Dosco
// call site. Better Stack patterns `17a0ce67…` (addListener, 21 occ.) and
// `3a6b00dc…` (emit, 4 occ.), Dosco Frontend (prod, application_id 2346967),
// 0 identified users, first/last 2026-07-14, call site `app:///inpage.js`,
// request URL `https://dosco.example.com/` (marketing homepage), Chrome 150.
//
...
// 0f78b2f8e9efa79fe9b2ea534e275c704f113eafea86bae5470f33174ebacebc, Dosco
...
// `https://dosco.example.com/auth`, Chrome 150.
//
 ...
 * running on `https://dosco.example.com/`); its OWN
...
// in first-party Dosco code: the `did=undefined` query param is the SDK's
...
// (Dosco Frontend prod, application_id 2346967): `SyntaxError`, message
...
// `https://dosco.example.com/auth` (auth page — the consent banner loads there
 ...
 * Dosco code. The `app:///injectedScript.bundle.js` source is a synthetic
 ...
 * request URL `https://dosco.example.com/auth?redirect=%2Fprojects%2F…`,
...
//
// Better Stack patterns (Dosco Frontend prod, application_id 2346967) — TWO
...
// (Dosco Frontend prod, application_id 2346967): 7 occurrences in the
...
//
// Better Stack patterns (Dosco Frontend prod, application_id 2346967) — all
...
// (Dosco Frontend prod, application_id 2346967): 1 occurrence ever (90-day
...
// (Dosco Frontend prod, application_id 2346967): `UnhandledRejection`, 1
...
// `https://dosco.example.com/` (the marketing/landing page). Stack trace: NONE —
...
// (Dosco Frontend prod, application_id 2346967): `OperationError`, 2
...
// on `https://dosco.example.com/projects/<id>` reached from Google account sign-in
...
// `0f03b24e…`).
//
// `OperationError` is the WebIDL type for async DOM operations, NOT a Dosco
...
// (Dosco Frontend prod, application_id 2346967): `UnhandledRejection`, 2
...
// `https://dosco.example.com/auth` (first occurrence) and
// `https://dosco.example.com/projects/c5a6e2f5-8880-4c30-bbbf-40fbcc1a1fbf` (second
...
// (Dosco Frontend prod, application_id 2346967): `UnhandledRejection`,
...
// `https://dosco.example.com/#error=access_denied&error_code=otp_expired&error_
...
// (Dosco Frontend prod, application_id 2346967): `Error`, 1 occurrence / 0
// identified users, last 2026-07-23 16:44:09 UTC, release
// `470fe6f3c88460212c3b187f6f86fb4ad456c4d6` (v0.10.13), transaction
// `/dashboard`, URL `https://dosco.example.com/dashboard`, mechanism
...
// (Dosco Frontend prod, application_id 2346967): all `SecurityError: The
...
// `https://dosco.example.com/`, browser Safari 26.6 on iOS (iPhone) 18.7, mechanism
...
// (`https://dosco.example.com/projects/834686a1-…/sessions/54f7abe9-…`), browser
...
//     `https://dosco.example.com/projects/e1d956a3-…/sessions/be897489-…` (session
...
// (Dosco Frontend prod, application_id 2346967): `ReferenceError`, message
...
// `https://dosco.example.com/` (marketing homepage), browser iPhone iOS 18.7 Safari
...
// script — NEVER in first-party Dosco code — and surfaces as an UNCAUGHT global
...
// `https://dosco.example.com/projects/61df2bc0-…` (project page), browser Chrome on
 ...
 * automation script, never first-party Dosco code. Requires the EXACT message
...
// (Dosco Frontend prod, application_id 2346967): `TimeoutError`, message
// `signal timed out`, 24 occurrences / 0 identified users, first 2026-05-16
// (recurring), last 2026-08-12 04:08:45 UTC, mechanism
// `auto.browser.global_handlers.onunhandledrejection` (UNCAUGHT,
// `handled:false`), request URL
// `https://dosco.example.com/projects/…/sessions/…` (session pages), browser Chrome
