...
describe('agent discovery documents', () => {
  test('publishes an RFC 9727 linkset with working Dosco relations', async () => {
    ...
    expect(body.linkset[0]['service-doc'][0].href).toBe('https://dosco.example.com/docs');
  ...
  test('publishes truthful OAuth discovery and protected resource metadata', async () => {
    ...
    const resource = (await getProtectedResource(
      new Request('https://dosco.example.com/.well-known/oauth-protected-resource'),
    ).json()) as any;
    expect(resource.resource).toBe('https://dosco.example.com');
  ...
  test('publishes an MCP discovery card for the real public endpoint', async () => {
    ...
    expect(card.transport).toEqual({
      ...
      endpoint: 'https://dosco.example.com/mcp',
    });
  ...
  test('negotiates the homepage to a Markdown route before auth middleware', async () => {
    const request = new NextRequest('https://dosco.example.com/', {
      headers: { Accept: 'text/markdown' },
    });
    const rewritten = await middleware(request);
    expect(rewritten.headers.get('x-middleware-rewrite')).toBe(
      'https://dosco.example.com/markdown-negotiation?path=%2F',
    );

    const response = getNegotiatedMarkdown(
      new Request('https://dosco.example.com/markdown-negotiation?path=%2F'),
    );
    expect(response.status).toBe(200);
  ...
  test('adds agent discovery links to the homepage middleware response', async () => {
    const response = await middleware(new NextRequest('https://dosco.example.com/'));
