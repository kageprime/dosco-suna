...
describe('scrape web helpers', () => {
  ...
  test('buildScrapeFailureResults maps per-url errors from aggregate message', () => {
    const output =
      'Error: Failed to scrape all 1 URLs. https://dosco.example.com: timeout of 35000ms exceeded';
    const results = buildScrapeFailureResults(output, ['https://dosco.example.com']);
    expect(results).toHaveLength(1);
    expect(results[0]).toMatchObject({
      url: 'https://dosco.example.com',
      success: false,
  ...
  test('resolveScrapeResults falls back to input urls on plain error output', () => {
    const output =
      'Error: Failed to scrape all 1 URLs. https://dosco.example.com: timeout of 35000ms exceeded';
    const results = resolveScrapeResults(output, { urls: 'https://dosco.example.com' });
    expect(results).toHaveLength(1);
