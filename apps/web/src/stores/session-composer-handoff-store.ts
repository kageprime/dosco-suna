...
interface CarriedDraftState {
  /** Session id → the draft waiting to be handed to that session's
   *  real composer. */
...
interface FirstPromptPreviewState {
  /** Session id → the first prompt's text, for RENDER only. */
 ...
 * same text, keyed by the session id, gone with the tab (a reload has
...
interface HeldSendFailureState {
  /** Session id → message id → the painted send whose upload failed. */
...
// too: the queue is the server's prompt inbox, keyed by the session id,
