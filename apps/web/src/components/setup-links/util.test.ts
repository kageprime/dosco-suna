...
describe('parseSetupLinkHref', () => {
  test('parses a same-origin secret-intake URL', () => {
    withWindowOrigin('https://dosco.example.com');
    expect(parseSetupLinkHref(`https://dosco.example.com/secret-intake/${TOKEN}`)).toEqual({
      kind: 'secret',
      token: TOKEN,
    });
  });
  ...
  test('unrelated URLs are ignored', () => {
    expect(parseSetupLinkHref('https://dosco.example.com/docs')).toBeNull();
...
describe('setupLinkChipLabel', () => {
  test('a raw URL as link text falls back to the friendly label', () => {
    expect(
      setupLinkChipLabel(`https://dosco.example.com/secret-intake/${TOKEN}`, TOKEN, 'Enter credentials'),
