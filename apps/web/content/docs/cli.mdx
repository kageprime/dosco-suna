...
The `kortix` command line interface (CLI) controls the platform from a terminal — your laptop or a session sandbox. This page shows the everyday dev loop, then lists every stable command and flag.
...
The platform stores authentication per host, not globally. A host is one API endpoint. Four hosts exist by default: `cloud` (Cloud), `selfhost` (your self-hosted stack), `local-dev`, and `kortix-internal-dev`. You can add more.
...
This command writes `.kortix/link.json` in the current directory. The platform reads this file to find your project on every command run from this folder. If you plan to run `kortix ship` first, skip this step. It links a new project for you when none exists.
</Step>

<Step title="Ship your code">
```sh
kortix ship
```
...
<Step title="Run and attach to sessions">
...
<Step title="Review with change requests">
...
| `kortix members invite <email> --role admin\|member [--project <id>:<role>]` | Invite by email. An existing user is added immediately; anyone else is mailed an invite link. `--project` is repeatable and applies on accept. Needs `member.invite`. |
...
| `kortix projects clone [<id>] [dir]` | Clone a project's repo through the authenticated git proxy. |
...
An existing GitHub `origin` links through the GitHub App. Any other existing `origin` is registered as-is. No `origin` creates a managed git repo.
...
| `kortix sessions share <id> [--mode private\|project\|members] [--member <id\|email>] [--group <id>] [--show] [--json]` | Set who inside the platform can open this session. With no `--mode` it prints the current setting and changes nothing. `--member` and `--group` are repeatable. |
...
Review Center. On for every project; `project.review.read` and
`project.review.act` gate reading and acting.
...
| `kortix connectors apps [<query>] [--category <c>] [--cursor <c>] [--limit <n>] [--json]` | Search the Composio toolkit catalog — the slugs `add --provider composio --app <slug>` accepts. Alias: `kortix connectors toolkits`. Add `--pipedream` for the legacy Pipedream catalog. |
...
Browse the marketplace, and install an item into a project.
...
The system skills are the platform's own documentation. They cover
...
| Command | Effect |
| --- | --- |
| `kortix system-skills list` | List the system skills. Default subcommand. |
...
A person, a group, or an agent gets roles from the platform; an agent additionally
carries CLI scopes in `kortix.yaml`, and a session can only do what both
...
A custom role only adds permissions. The platform has no deny rule, so a role cannot
