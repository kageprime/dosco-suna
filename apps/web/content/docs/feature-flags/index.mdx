...
kortix projects features disable teams
...
| `pi_worker` | Pi Worker Runtime (compiled) | Experimental | Off | `routes` |
...
- **`llm_gateway`** — route the project through the managed LLM
...
- **`pi_worker`** — compile a pi-based worker runtime and the OpenCode
  compiled-boot artifacts on every push. Sessions boot on the worker only when
  the manifest also sets `runtime: pi`.

### Graduated flags

A graduated feature is on for every project and has no switch.

- **Review Center** graduated out of the flag system. The Review tab, the
...
|---|---|
| `teams`, `agentmail_email` | Kortix re-runs channel-connector materialization, so the connector appears or disappears with the flag. |
