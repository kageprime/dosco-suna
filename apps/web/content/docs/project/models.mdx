---
title: Models
description: How the platform picks a model, and how billing works for managed vs. your-own-key models.
---

The platform runs each [session](/docs/work/sessions) on a model. This page
...
Set no model, and the platform resolves one through five layers, in order. (The
...
The platform uses the first layer that has a value it can still serve. A saved
...
- **Platform fee.** The platform adds a 10% fee, billed as credits, on top of what
...
- **Silent failover.** If your BYOK key hits a rate limit or billing error
  mid-turn, the platform retries on a managed model and bills your credits instead
