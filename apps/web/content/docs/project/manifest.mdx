...
A project has two configuration surfaces with strict, non-overlapping ownership.

- **Config** — `kortix.yaml` at the repo root, plus `.kortix/Dockerfile`. The platform reads this: the trigger sweep, the sandbox builder, session token minting, and the dashboard.
...
Any repo with a valid manifest at its root is a project. `kortix.yaml` (v2, YAML only) is the current format for new projects, created by the web "Create project" flow and by `kortix init`. `kortix.toml` (v1) still works for existing projects, but the platform accepts no new v2 features on it. See [Legacy TOML](/docs/project/legacy-toml) for the v1 schema and the v1-to-v2 migration steps.
...
This block is the **second of the two bindings**: a principal gets roles from the platform, and an agent additionally carries the CLI scopes declared here. A session can only do what both allow. See [One vocabulary, two bindings](/docs/accounts#one-vocabulary-two-bindings).
...
| `kortix_cli`          | `none`  | Which CLI and API permissions this agent may exercise. `all` means everything the launching user's roles allow — an agent can never exceed its launcher.                                            |
| `skills`              | `none`  | Which skills this agent may load.                                                                                                                                                                         |
| `repository_access` | `true` | Whether new sessions receive the project repository. `false` disables repository delivery and repository access through session credentials. Existing sessions keep their saved policy. |
...
Set `agents.<name>.repository_access` to `false` for agents that do not need project source files.
They can still create sandbox files and use their granted tools, secrets, and connectors.
Set it to `true` for agents that need the repository. Git permissions remain separate.
Change this setting in the agent editor under **Workspace → Project repository access**.
The setting applies to new sessions. Existing sessions keep their saved policy, including after runtime replacement.
...
The deprecated `workspace` field remains readable: `branch` means `true`, and `runtime` means `false`.
Legacy `read` configurations still reject session creation until you replace the field with an explicit boolean.
There is no read-only repository mode. Conflicting old and new fields are rejected.
The editor and SDK send `repository_access`. For disabled access, the API also stores `workspace: runtime` as a compatibility mirror. This preserves the restriction for older API replicas during rollout or rollback. API reads omit the mirror. Hand-authored manifests using only `repository_access` require the updated API.
