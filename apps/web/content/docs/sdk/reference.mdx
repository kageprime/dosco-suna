...
kortix.gitBackend; // the instance git backend ("Kortix managed")
...
The platform has one grant record: an **assignment**. It binds one principal (`user`,
...
| `listLinkableInstallations(input)` | installs the authorized GitHub user may link; each entry carries `linked_to_other_accounts`, a count of other Kortix accounts holding it — never their names |
...
| `linkRepository(input)` | import a repo as a project. `source: 'managed'` imports one the instance git backend holds (self-host operator only, 403 otherwise) and is mutually exclusive with `installation_id` (400) |

This list holds real account connections only. "Kortix managed" is not one of
them — it is the instance backend below.

### Instance git backend — `kortix.gitBackend`

One deployment-wide owner plus credential, used for Kortix-managed
repositories. It is never an account connection and never appears in
`kortix.github.listInstallations`.

| method | what |
| --- | --- |
| `get()` | `{ configured, kind: 'app' | 'pat' | null, owner }` — readable by any authenticated user; it exposes only the owner login that every managed `repo_url` already carries |
| `repositories({ search?, limit? })` | repositories under that owner. Self-host operators only: on cloud that owner holds every customer's project repo, so the API answers `403` for anyone else |
...
and the platform substitutes the real value outside the sandbox, for the exact HTTPS
...
await files.upload(file, '/workspace/uploads', file.name, {
...
Uploads larger than 64 KiB cross the sandbox edge in verified chunks. The
progress callback runs after each chunk reaches the session filesystem.
...
ChatGPT/Codex subscription messages contribute `$0.00`, including historical
messages with nonzero runtime costs. Token and context usage remain visible.
Mixed sessions retain charges for paid API messages. OpenAI API models remain
billable, including API models whose names contain `codex`.
...
| `@kortix/sdk/internal/*` | Internal | Used by the web app only. Not a supported API. |
...
Read `kortix.projects.modelAccess(projectId)`. Change one target with `kortix.projects.setModelAccess(projectId, { target: 'provider' | 'model', id, enabled })`.

Use provider ID `kortix` for Kortix Managed Models, or a catalog provider ID for BYOK, Codex, and custom providers. Use the full gateway wire ID for a model. Provider disable covers future models and preserves individual model preferences and saved credentials.

The current default model and its provider require a replacement before disabling (`409 cannot_disable_default`). Direct gateway requests to disabled targets fail with `model_disabled` or `provider_disabled`. Existing in-flight upstream requests finish normally. The response's `enforced` flag is false when the project uses native OpenCode instead of the gateway.

React consumers use `useModelAccess(projectId)` from `@kortix/sdk/react`. Its `setEnabled(change)` mutation refreshes the management catalog and session picker.
