...
OpenCode session in the host. The platform creates and persists the root session.
...
agent persisted on the session. This prevents a snapshot-inherited
...
3. The persisted session default.
...
`s.stream()` is the OpenCode REST compatibility event stream. The API
...
| `AuthError`            | `ApiError` | `getToken` returned `null`. The platform never sent the request                      | `code` is always `'NO_SESSION'`                                      |
...
The platform retries some requests before your code sees an error. If a `GET` or `HEAD` request returns `502`, `503`, or `504`, the platform retries it up to 2 times, with a 250ms then 500ms delay. A transient transport failure on a `GET` or `HEAD` — a network error, not a status code — is retried the same way. A retry that succeeds never reaches `onError`. The platform never retries `POST`, `PUT`, `PATCH`, or `DELETE` requests, or a `500` response.

The platform throws `AuthError` on the client, before it sends a request, when `getToken()` returns `null`. `AuthError` extends `ApiError`, so `err instanceof ApiError` still matches. Check `err instanceof AuthError`, or `err.code === 'NO_SESSION'`, to treat "not signed in" as a separate case from a backend failure.

The platform throws `BillingError` for every HTTP `402` response: out of credits, over a plan limit, or another billing gate. `detail.message` holds the reason from the backend.

The platform throws `RequestTooLargeError` for HTTP `431`. This usually means the request carried too many files. `detail.suggestion` holds a ready-to-show hint for the user.
