...
A session runs in an isolated sandbox on Daytona, Platinum, or E2B Cloud, built from a layered image. This page is the reference for the sandbox lifecycle, the environment the platform injects at boot, the token families, and the sandbox image itself. For the session concept, see [Sessions](/docs/work/sessions).
...
| Status | Set when |
| --- | --- |
| `provisioning` | At session create. The platform creates the session branch and requests the sandbox. |
...
The platform enforces a concurrent-session limit per account. Exceeding your tier's limit returns `429`.
...
- The platform cuts the branch from `base_ref`, which defaults to the project's default branch, at session-create time.
...
Use **Ask Agent: Sync Branch & Reload** from the session command palette when the base branch changed. The agent inspects the session branch, preserves local work, fetches the latest `base_ref`, resolves conflicts, runs the relevant tests, and commits the reconciliation. The platform does not choose one side of a conflict or reset the working tree.
...
The platform injects these variables at boot. Only a project secret explicitly configured for `runtime` delivery enters the sandbox. Connector and model-provider credentials stay on the server.
...
| `KORTIX_LLM_BASE_URL` | LLM-gateway base URL. The gateway resolves provider credentials server-side. |
| `KORTIX_TOKEN` | The session-bound credential. See below. |

The platform does not inject `KORTIX_WORKSPACE`. The image bakes in `/workspace` and no per-session step sets it. The platform does not inject a git token either — the daemon fetches a short-lived clone credential when it needs one; see [Pushing from a session](#pushing-from-a-session) below. The platform rejects a user secret named with the `KORTIX_*` prefix, because it reserves that prefix for platform variables.

## Session credential

The platform has external tokens you create yourself and one credential inside each sandbox.

### External tokens

A personal access token (PAT, prefixed `kortix_pat_`) or a service account (prefixed `kortix_sa_`) authenticates calls to the API, the SDK, and the CLI from outside a sandbox. See [Authentication](/docs/sdk/auth) for PAT scope, service-account setup, and how to choose between them.

### In-sandbox token

`KORTIX_TOKEN` is minted when the platform starts the session environment. It is bound to the launching user, project, session, and agent grant. The daemon, CLI, Git credential helper, LLM gateway, and connector gateway use this same credential. Each API route still applies its own capability check.
...
The daemon sends `KORTIX_TOKEN` only to the Git proxy. The proxy resolves the upstream Git credential on the server. No upstream Git token enters the sandbox. `git push origin HEAD` sends commits to the session branch. Landing on the default branch requires a merged [change request](/docs/work/change-requests).
...
Every sandbox boots from an image built in two layers. Your Dockerfile defines the base environment. The runtime layer is added on top, so the dashboard can connect to the sandbox.

```
┌─────────────────────────────────────────┐
│  runtime layer (added on top)    │  ← opencode + kortix-agent + entrypoint
├─────────────────────────────────────────┤
│  Your Dockerfile                        │  ← .kortix/Dockerfile
└─────────────────────────────────────────┘
```

If your project has no Dockerfile, the platform builds sessions from a bare `ubuntu:24.04` image plus the runtime layer below.
...
Everything you install in your own Dockerfile stays on `PATH`. The platform does not remove or relocate it.
...
| Don't bake credentials into the image. | Declare the name in `env:` and set the value as a project secret. The platform injects it at session start. |
...
`cpu` takes 1–32 cores, `memory` takes 1–128 GiB, and `disk` takes 1–500 GiB. Kortix clamps any value above these limits down to the limit. The platform does not support GPUs; a `gpu` key on a template produces a warning, not an error.
