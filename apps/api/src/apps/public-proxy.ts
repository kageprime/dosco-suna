...
const PUBLIC_STATUS_COPY: Record<PublicAppStatus, {
  ...
  validating: {
    ...
    message: 'The platform is checking the source and deployment configuration.',
    code: 'app_deployment_validating',
    progress: true,
  },
  building: {
    title: 'Building your App',
    message: 'The platform is producing an immutable runtime image.',
    code: 'app_deployment_building',
    progress: true,
  },
  provisioning: {
    title: 'Provisioning your App',
    message: 'The platform is creating the serverless runtime.',
    code: 'app_deployment_provisioning',
    progress: true,
  },
  checking: {
    title: 'Checking readiness',
    message: 'The platform is waiting for the App to accept traffic.',
    code: 'app_deployment_checking',
    progress: true,
  },
  ready: {
    title: 'Activating your App',
    message: 'The deployment is ready. The platform is assigning stable traffic.',
    code: 'app_deployment_activating',
    progress: true,
  },
  starting: {
    title: 'Starting your App',
    message: 'The platform is resuming the serverless runtime. This page will continue automatically.',
    code: 'app_starting',
    progress: true,
  },
  budget: {
    title: 'App paused',
    message: 'This App reached its monthly compute limit. The owner can increase the limit in Apps.',
  ...
  unfunded: {
    title: 'App paused',
    message: 'This account cannot start compute right now. The owner can restore it in Billing.',
  ...
  capacity: {
    title: 'App paused',
    message: 'This account is already running its maximum number of Apps. The owner can stop one in Apps.',
  ...
  failed: {
    ...
    message: 'Open Apps or run kortix apps logs to inspect the deployment.',
...
<body><main class="card"><div class="mark"><span class="glyph"></span>Apps</div><div class="state"><span class="dot"></span>${escapeHtml(status)}</div><h1>${heading}</h1><p>${escapeHtml(copy.message)}</p></main></body></html>`, {
    status: httpStatus,
    headers,
  });
}
...
/**
 * A credential presented as a bearer token, resolved to the identity it
...
export interface AppViewerHeaders {
  ...
  context: string;
  /** The viewer's App-scoped token. Only for `viewer_token_scope: 'api'`. */
  token: string | null;
}
...
export async function appViewerEndpointResponse(
  ...
  if (!userId && app.accessMode !== 'password') {
    // A server-side caller inside the App can present its own credential
    ...
    return Response.json(
      {
        ...
        error_description:
          app.accessMode === 'public' || app.accessMode === 'password'
            ? `A ${app.accessMode} App has no signed-in viewer.`
...
export async function authorizeAppRequest(
  ...
  // A credential, for callers that are not browsers. Checked after the
