/**
 * E2E for the connector HTTP surface — drives the REAL Hono router + REAL gateway
...
function principalFor(userId: string): ConnectorPrincipal {
  return {
    userId,
    accountId: ACCOUNT,
    projectId: PROJECT,
...
function catalogFor(p: ConnectorPrincipal): CatalogConnector[] {
...
const deps: ConnectorRouterDeps = {
  ...
  listDiscoverConnectors: async ({ q, cursor }) => ({
...
describe('GET /catalog', () => {
  test('lists usable connectors + actions', async () => {
    const json = await (await req('/catalog', { headers: { 'x-test-user': ALICE } })).json();
  ...
  test('connector with no credential hidden until connected', async () => {
    ...
    expect(
      (await (await req('/catalog', { headers: { 'x-test-user': ALICE } })).json()).connectors,
    ).toHaveLength(0);
  });
});
