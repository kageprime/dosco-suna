...
const FLAGS: readonly FeatureFlagDef[] = [
  ...
  },
  {
    ...
    description:
      'Assign AgentMail inbox connections to the agent so inbound email threads can start and continue sessions. Native email channels are still experimental.',
    stability: 'experimental',
    available: () => true,
  ...
  },
  {
    ...
    description:
      'Connect a Microsoft Teams bot so chats and channels can start and continue sessions. The install flow, org-catalog publishing, and bring-your-own-bot setup are still experimental.',
    stability: 'experimental',
  ...
  },
  {
    ...
    name: 'LLM Gateway',
    description:
      'Route this project through the managed LLM gateway (managed models, metering, budgets). Off, the sandbox runs native OpenCode model management: your provider API keys are injected as ordinary env vars and models are native provider/model refs. Toggling refreshes active sandboxes either way.',
    stability: 'experimental',
  ...
  },
  {
    ...
    description:
      'Let a secret be enforced at the network instead of loaded into the sandbox: the sandbox holds a handle and the platform substitutes the real value only on requests to approved hosts. Off ⇒ every secret loads into the sandbox environment and the "Enforce at the network" option is hidden.',
    stability: 'experimental',
    available: () => true,
