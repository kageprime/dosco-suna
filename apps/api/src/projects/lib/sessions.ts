...
export async function buildSessionSandboxEnvVars(input: {
  ...
  return {
    ...
    // on-demand fetches, which stall through the git proxy when its
    ...
    ...buildSessionRuntimeEnv({
      ...
      // Every sandbox clones through the Git proxy with KORTIX_TOKEN.
...
/**
 * The git-proxy origin for a project — the UNIVERSAL client-facing git
 * URL. Clients clone/push this with a token; the API resolves the real
