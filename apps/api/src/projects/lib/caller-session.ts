 ...
 * interchangeable:
 *
 *   - `authType: 'pat'`      → a Dosco PROJECT SESSION id (a sandbox connector
 ...
 *
 * Reading the raw context var and calling it a Dosco session is a live bug, not
...
export function callerKortixSessionId(c: Context): string | null {
  // Anything that is not a Supabase browser JWT carries a real Dosco session id
