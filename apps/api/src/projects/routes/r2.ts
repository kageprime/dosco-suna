...
// then registers it as a project.
...
projectsApp.openapi(
  ...
  if (!githubAuth.installation || !githubAuth.auth) {
    return c.json({
      error: 'Install the GitHub App before creating GitHub-backed projects',
  ...
  // Commit the starter into the fresh repo so users land with a
