...
beforeEach(() => {
  ...
  configState.KORTIX_URL = 'https://dev-api.dosco.live';
...
describe('previewUrlTemplate', () => {
  test('carries the shape to the client with only two slots', () => {
    ...
    expect(previewUrlTemplate()).toBe('https://dev-p{port}-{sandbox}.p.dosco.live');
  });

  test('each environment gets its own label under one wildcard', () => {
    configState.KORTIX_PREVIEW_BASE_DOMAIN = 'p.dosco.live';
    configState.INTERNAL_KORTIX_ENV = 'prod';
    expect(previewUrlTemplate()).toBe('https://prod-p{port}-{sandbox}.p.dosco.live');
  });
...
describe('resolvePreviewHost', () => {
  ...
  test('round-trips the hostname the template describes', () => {
    expect(resolvePreviewHost(`dev-p8081-${LABEL}.p.dosco.live`)).toEqual({
      port: 8081,
      sandboxLabel: LABEL,
      local: false,
    });
  ...
  test('accepts a Host header carrying a port and a trailing dot', () => {
    expect(resolvePreviewHost(`dev-p8081-${LABEL}.p.dosco.live.:443`)).toEqual({
      port: 8081,
      sandboxLabel: LABEL,
      local: false,
    });
  ...
  test('refuses another environment’s label', () => {
    expect(resolvePreviewHost(`prod-p8081-${LABEL}.p.dosco.live`)).toBeNull();
  });
  test('refuses a nested label that would escape the wildcard certificate', () => {
    expect(resolvePreviewHost(`a.dev-p8081-${LABEL}.p.dosco.live`)).toBeNull();
  });
  test('refuses the API host itself and unrelated hosts', () => {
    expect(resolvePreviewHost('dev-api.dosco.live')).toBeNull();
    expect(resolvePreviewHost('evil.com')).toBeNull();
    expect(resolvePreviewHost('')).toBeNull();
  });
  test('refuses a malformed label', () => {
    expect(resolvePreviewHost('dev-p-abc.p.dosco.live')).toBeNull();
    expect(resolvePreviewHost('dev-8081-abc.p.dosco.live')).toBeNull();
    expect(resolvePreviewHost(`dev-p99999999-${LABEL}.p.dosco.live`)).toBeNull();
  });
});
...
describe('previewOriginFor', () => {
  test('builds the full origin a shared preview is served on', () => {
    configState.KORTIX_PREVIEW_BASE_DOMAIN = 'p.dosco.live';
    expect(previewOriginFor(SBX, 8081)).toBe(`https://dev-p8081-${LABEL}.p.dosco.live`);
  });
...
describe('warnIfPreviewOriginsMissing', () => {
  ...
  test('warns on a public deployment with no preview domain — the silent-downgrade case', () => {
    configState.KORTIX_URL = 'https://api.essentia.dosco.live';
    configState.KORTIX_PREVIEW_BASE_DOMAIN = undefined;
    const log = spy();
    warnIfPreviewOriginsMissing(log);
    expect(log.calls).toHaveLength(1);
    expect(log.calls[0]!.message).toContain('no KORTIX_PREVIEW_BASE_DOMAIN');
    expect(log.calls[0]!.meta?.kortix_url).toBe('https://api.essentia.dosco.live');
  });

  test('silent once a preview domain is configured', () => {
    configState.KORTIX_URL = 'https://api.essentia.dosco.live';
    configState.KORTIX_PREVIEW_BASE_DOMAIN = 'p.essentia.dosco.live';
    const log = spy();
    warnIfPreviewOriginsMissing(log);
