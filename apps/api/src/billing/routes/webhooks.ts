...
import { processPaystackWebhook } from '../services/paystack';
...
// Paystack signs with HMAC-SHA512 over the raw body (x-paystack-signature).
// Same raw-body discipline as the Stripe route: no zod body schema, no bearer.
webhooksRouter.openapi(
  createRoute({
    method: 'post',
    path: '/paystack',
    tags: ['billing'],
    summary: 'Paystack webhook (HMAC-SHA512 signature-verified, public)',
    responses: {
      200: json(z.record(z.string(), z.any()), 'Webhook processing result'),
      ...errors(400, 500),
    },
